...
def compute_threshold_acc_on_sentence_level_emb(emb_txt, emb_com, threshold=0.1):
...
def compute_combined_embeddings(model, texts):
    ...
    emb_com = emb_sub + emb_rel + emb_obj
    return emb_com
...
for epoch in range(config.epochs):
    ...
    logger.info(f"Epoch {epoch} thrs0.1 training acc *100: {100*np.mean(train_acc_l):.2f}")
    ...
    logger.info(f"Epoch {epoch} thrs0.1 Val acc *100: {100*np.mean(val_acc_l):.2f}")
    ...
    logger.info(f"Epoch {epoch} thrs0.1 Gen acc *100: {100*np.mean(gen_acc_l):.2f}")
    wandb.log({
