...
if __name__ == "__main__":
    ...
    wandb.init(
        ...
        entity="karry-z",
        name="train_CLIP_combined_compositions",
    ...
    model.noun_projection = torch.nn.Sequential(
    ...
    model.rel_projection = torch.nn.Sequential(
        torch.nn.Linear(config.emb_dim, 300),
        torch.nn.ReLU(),
        torch.nn.Linear(300, 300),
        torch.nn.ReLU(),
        torch.nn.Linear(300, config.emb_dim-200),
    ) 
    ...
    def compute_combined_embeddings(model, texts):
        ...
        emb_com = model.mlp(torch.cat([model.noun_projection(emb_sub), model.rel_projection(emb_rel), model.noun_projection(emb_obj)], dim=-1))
        return emb_com
    ...
    def forward_(self, batch_images, texts):
        ...
        text_features = compute_combined_embeddings(self, texts)
