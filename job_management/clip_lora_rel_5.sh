...
#SBATCH --job-name=clip_lora_rel_5
...
# "--save_model"
srun -u python train_lora.py --model_name clip \
    --dataset rel \
    --epochs 5 \
    --save_dir /user/work/pu22650/clip-binding-out/clip_lora_rel_5 \
    --save_model \
    --train_batch_size 16 \
    --eval_batch_size 32 
else
...
srun --pty -p gpu_short --gres=gpu:1 --mem=64G -A EMAT028344 python -m debugpy --listen 0.0.0.0:5678 --wait-for-client train_lora.py --model_name clip \
    --dataset rel \
    --epochs 5 \
    --save_dir /user/home/pu22650/clip-binding/out/clip_lora_rel_5 \
    --save_model \
    --train_batch_size 8 \
    --eval_batch_size 16 
fi
