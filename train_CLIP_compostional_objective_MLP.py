...
wandb.init(
    ...
    entity="karry-z",
    name="train_CLIP_compostional_objective_MLP",
...
def train_model(model, optimizer, dataset_dict, config, device):
    ...
    model.train()
    model.to(device)
    loss_contrastive = CrossEntropyLoss()
    loss_compostional = nn.MSELoss()
    ...
    for i in range(config.epochs):
        ...
        for bid, batch in enumerate(train_dataloader):
            ...
            batch_img, texts, labels = batch
            logits_per_image, emb_txt, emb_com = model(batch_img, texts)
            batch_target = labels.to(device)
            l1 = loss_contrastive(logits_per_image, batch_target)
            # sigmoid on l1
            l1 = config.w_l1 * torch.sigmoid(l1)
            if l1.isnan():
                l1 = torch.tensor(0.1).to(device).to(l1.dtype)
            # l2 = loss_compostional(emb_txt, emb_com, torch.ones(emb_com.shape[0]).to(device))
            l2 = loss_compostional(emb_txt, emb_com)
            l2 = (1 - config.w_l1) * torch.sigmoid(l2)
            if l2.isnan():
                l2 = torch.tensor(0.1).to(device)
            loss =  l1 +  l2
            logger.info(f"contrastive loss: {l1:.2f}; compostional loss: {l2:.2f}")
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--model_name", help="name of the experiment", type=str, default="comp_clip"
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--train_batch_size", help="train batch size", default=10, type=int
    )
    parser.add_argument(
